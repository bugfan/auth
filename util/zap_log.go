...
func InitLog() {
	...
	if logPath := goini.Env.Getenv("LOG_PATH_FILE"); logPath != "" {
		cfg.OutputPaths = []string{logPath}
	} else {
		cfg.OutputPaths = []string{"./ZAP_LOG.txt"}
	}
...
func Zlog(logLevel, logType, msg, ctx string) {
	...
	log.Println(msg, "level", logLevel, "category", logType, "ctx", ctx, "ts", curDate, "caller", caller)
