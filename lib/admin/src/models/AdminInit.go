...
type AdminInteFunc func(o orm.Ormer)

var callback AdminInteFunc = defaultInsertFunc
...
func SetAdminInteCallBack(f AdminInteFunc) {
	callback = f
}
...
func defaultInsertFunc(o orm.Ormer) {
	insertUser()
	insertGroup()
	insertRole()
	insertNodes()
...
func insertNodes() {
	...
	//nodes := make([20]Node)
	nodes := [24]Node{
		{Name: "rbac", Title: "管理", Remark: "", Level: 1, Pid: 0, Status: 2, Group: g},
