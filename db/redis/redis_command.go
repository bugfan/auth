...
func (rc *Redis) GetRaw(key string) ([]byte, error) {
	...
	if err != nil && err != redis.Nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_GetRaw", err.Error(), fmt.Sprintf("key: %s", key))
	}
	return c, err
}
...
func (rc *Redis) MGet(keys ...string) []string {
	...
	if err := sliceObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_MGet", err.Error(), fmt.Sprintf("keys: %s", strings.Join(keys, ",")))
		return []string{}
	}
	tmp := sliceObj.Val()
...
func (rc *Redis) MGets(keys []string) ([]interface{}, error) {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_MGets", err.Error(), fmt.Sprintf("keys: %s", strings.Join(keys, ",")))
	}
	return ret, err
}
...
func (rc *Redis) Set(key string, value interface{}, expire time.Duration) bool {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_Set", err.Error(), fmt.Sprintf("key: %s", key))
		return false
	}
	return true
}
...
func (rc *Redis) HGet(key string, fields string) (string, error) {
	...
	err := strObj.Err()
	if err != nil && err != redis.Nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_HGet", err.Error(), fmt.Sprintf("key: %s, field: %s", key, fields))
		return "", err
	}
	if err == redis.Nil {
		return "", nil
...
func (rc *Redis) HMGetMap(key string, fields []string) map[string]string {
	...
	if err := sliceObj.Err(); err != nil && err != redis.Nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_HMGetMap", err.Error(), fmt.Sprintf("key: %s, fields: %s", key, strings.Join(fields, ",")))
		return make(map[string]string)
	}
...
func (rc *Redis) HMSet(key string, hash map[string]string, expire time.Duration) bool {
	if len(hash) > 0 {
		...
		if err != nil {
			...
			util.Zlog(util.LogLevelWarn, "redis_exec_HMSet", err.Error(), fmt.Sprintf("key: %s, fields: %s", key, hashString))
			return false
		}
...
func (rc *Redis) HSet(key string, field string, value interface{}) bool {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_HSet", err.Error(), fmt.Sprintf("key: %s, field: %s", key, field))
		return false
	}
	return true
}
...
func (rc *Redis) HDel(key string, field ...string) bool {
	...
	if err := IntObj.Err(); err != nil {
		...
		util.Zlog(util.LogLevelWarn, "redis_exec_HDel", err.Error(), fmt.Sprintf("key: %s, field: %s", key, field))
		return false
	}
...
func (rc *Redis) SetNx(key string, value interface{}, expiration time.Duration) bool {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_SetNx", err.Error(), fmt.Sprintf("key: %s", key))
		return false
	}
...
func (rc *Redis) Incr(key string) bool {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_Incr", err.Error(), fmt.Sprintf("key: %s", key))
		return false
	}
	return true
}
...
func (rc *Redis) IncrBy(key string, increment int64) (int64, error) {
	...
	if err := intObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_IncrBy", err.Error(), fmt.Sprintf("key: %s", key))
		return 0, err
	}
...
func (rc *Redis) Decr(key string) bool {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_Decr", err.Error(), fmt.Sprintf("key: %s", key))
		return false
	}
	return true
}
...
func (rc *Redis) Type(key string) (string, error) {
	...
	if err := statusObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_Type", err.Error(), fmt.Sprintf("key: %s", key))
		return "", err
	}
...
func (rc *Redis) ZRevRange(key string, start, stop int64) ([]string, error) {
	...
	if err := strSliceObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_ZRevRange", err.Error(), fmt.Sprintf("key: %s, start: %d, stop: %d", key, start, stop))
		return []string{}, err
	}
...
func (rc *Redis) ZRevRangeWithScores(key string, start, stop int64) ([]redis.Z, error) {
	...
	if err := zSliceObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_ZRevRangeWithScores", err.Error(), fmt.Sprintf("key: %s, start: %d, stop: %d", key, start, stop))
		return []redis.Z{}, err
	}
...
func (rc *Redis) ZRange(key string, start, stop int64) ([]string, error) {
	...
	if err := strSliceObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_ZRange", err.Error(), fmt.Sprintf("key: %s, start: %d, stop: %d", key, start, stop))
		return []string{}, err
	}
	return strSliceObj.Val(), nil
}
...
func (rc *Redis) ZRevRank(key string, member string) (int64, error) {
	...
	if err := intObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_ZRevRank", err.Error(), fmt.Sprintf("key: %s, member: %s", key, member))
		return 0, err
	}
	return intObj.Val(), nil
}
...
func (rc *Redis) ZRevRangeByScore(key string, opt redis.ZRangeBy) ([]string, error) {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_ZRevRangeByScore", err.Error(), fmt.Sprintf("key: %s", key))
		return []string{}, err
	}
...
func (rc *Redis) ZRevRangeByScoreWithScores(key string, opt redis.ZRangeBy) ([]redis.Z, error) {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_ZRevRangeByScoreWithScores", err.Error(), fmt.Sprintf("key: %s", key))
		return []redis.Z{}, err
	}
...
func (rc *Redis) ZRangeByScoreWithScores(key string, opt redis.ZRangeBy) ([]redis.Z, error) {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_ZRangeByScoreWithScores", err.Error(), fmt.Sprintf("key: %s", key))
		return []redis.Z{}, err
	}
...
func (rc *Redis) HMGet(key string, fileds []string) []string {
	...
	if err := sliceObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_HMGet", err.Error(), fmt.Sprintf("key: %s, fields: %s", key, strings.Join(fileds, ",")))
		return []string{}
	}
	tmp := sliceObj.Val()
	strSlice := make([]string, 0, len(tmp))
...
func (rc *Redis) ZCard(key string) (int64, error) {
	...
	if err := IntObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_ZCard", err.Error(), fmt.Sprintf("key: %s", key))
		return 0, err
	}
	return IntObj.Val(), nil
}
...
func (rc *Redis) ZScore(key string, member string) (float64, error) {
	...
	err := FloatObj.Err()
	if err != nil && err != redis.Nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_ZScore", err.Error(), fmt.Sprintf("key: %s, member: %s", key, member))
		return 0, err
	}
...
func (rc *Redis) ZAdd(key string, members ...redis.Z) (int64, error) {
	...
	if err := IntObj.Err(); err != nil && err != redis.Nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_ZAdd", err.Error(), fmt.Sprintf("key: %s", key))
		return 0, err
	}
...
func (rc *Redis) ZCount(key string, min, max string) (int64, error) {
	...
	if err := IntObj.Err(); err != nil && err != redis.Nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_ZCount", err.Error(), fmt.Sprintf("key: %s, min: %s, max: %s", key, min, max))
		return 0, err
	}
...
func (rc *Redis) ZIncrBy(key, member string, increment float64) (float64, error) {
	...
	if err := IntObj.Err(); err != nil && err != redis.Nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_ZIncrBy", err.Error(), fmt.Sprintf("key: %s, member: %s, incr: %d", key, member, increment))
		return 0, err
	}
...
func (rc *Redis) Del(key string) int64 {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_Del", err.Error(), fmt.Sprintf("key: %s", key))
		return 0
	}
	return result
}
...
func (rc *Redis) HIncrBy(key string, field string, incr int) {
	...
	if err := result.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_HIncrBy", err.Error(), fmt.Sprintf("key: %s, fiels: %s, int: %d", key, field, incr))
	}
}
...
func (rc *Redis) Exists(key string) bool {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_Exists", err.Error(), fmt.Sprintf("key: %s", key))
		return false
	}
	return result
}
...
func (rc *Redis) LPush(key string, values ...interface{}) (int64, error) {
	...
	if err := IntObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_LPush", err.Error(), fmt.Sprintf("key: %s", key))
		return 0, err
	}
...
func (rc *Redis) RPush(key string, values ...interface{}) (int64, error) {
	...
	if err := IntObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_RPush", err.Error(), fmt.Sprintf("key: %s", key))
		return 0, err
	}
...
func (rc *Redis) RPop(key string) (string, error) {
	...
	if err := strObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_RPop", err.Error(), fmt.Sprintf("key: %s", key))
		return "", err
	}
...
func (rc *Redis) LRange(key string, start, stop int64) ([]string, error) {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_LRange", err.Error(), fmt.Sprintf("key: %s", key))
		return []string{}, err
	}
...
func (rc *Redis) LLen(key string) int64 {
	...
	if err := IntObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_LLen", err.Error(), fmt.Sprintf("key: %s", key))
		return 0
	}
...
func (rc *Redis) LRem(key string, count int64, value interface{}) int64 {
	...
	if err := IntObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_LRem", err.Error(), fmt.Sprintf("key: %s", key))
		return 0
	}
...
func (rc *Redis) LIndex(key string, idx int64) (string, error) {
	...
	if err != nil && err != redis.Nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_LIndex", err.Error(), fmt.Sprintf("key: %s", key))
	}
	return ret, err
}
...
func (rc *Redis) LTrim(key string, start, stop int64) (string, error) {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_LTrim", err.Error(), fmt.Sprintf("key: %s", key))
	}
	return ret, err
}
...
func (rc *Redis) ZRemRangeByRank(key string, start, stop int64) (int64, error) {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_ZRemRangeByRank", err.Error(), fmt.Sprintf("key: %s", key))
		return 0, err
	}
...
func (rc *Redis) Expire(key string, expiration time.Duration) (bool, error) {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_Expire", err.Error(), fmt.Sprintf("key: %s", key))
		return false, err
	}
...
func (rc *Redis) ZRem(key string, members ...interface{}) (int64, error) {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_ZRem", err.Error(), fmt.Sprintf("key: %s", key))
		return 0, err
	}
	return result, nil
}
...
func (rc *Redis) SAdd(key string, member ...interface{}) (int64, error) {
	...
	if err := intObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_SAdd", err.Error(), fmt.Sprintf("key: %s", key))
		return 0, err
	}
	return intObj.Val(), nil
}
...
func (rc *Redis) SMembers(key string) ([]string, error) {
	...
	if err := strSliceObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_SMembers", err.Error(), fmt.Sprintf("key: %s", key))
		return []string{}, err
	}
	return strSliceObj.Val(), nil
}
...
func (rc *Redis) SIsMember(key string, member interface{}) (bool, error) {
	...
	if err := boolObj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_SIsMember", err.Error(), fmt.Sprintf("key: %s", key))
		return false, err
	}
...
func (rc *Redis) SCard(key string) (int64, error) {
	...
	if err := obj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_SCard", err.Error(), fmt.Sprintf("key: %s", key))
		return 0, err
	}
	return obj.Val(), nil
}
...
func (rc *Redis) SrandMemberN(key string, count int64) ([]string, error) {
	...
	if err := obj.Err(); err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_SrandMemberN", err.Error(), fmt.Sprintf("key: %s", key))
		return []string{}, err
	}
	return obj.Val(), nil
}
...
func (rc *Redis) GeoAdd(key string, location *redis.GeoLocation) (int64, error) {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_GeoAdd", err.Error(), fmt.Sprintf("key: %s", key))
		return 0, err
	}
...
func (rc *Redis) GeoRadius(key string, longitude, latitude float64, query *redis.GeoRadiusQuery) ([]redis.GeoLocation, error) {
	...
	if err != nil {
		util.Zlog(util.LogLevelWarn, "redis_exec_GeoRadius", err.Error(), fmt.Sprintf("key: %s", key))
